$(function () {
    $.ajax({
        url: "/AdminDashBoard/LoadRegionalManagerDashboard",
        type: "get",
        datatype: "json",
        async: false,
        ...
        success: function (response) {
            if (response != null) {
                ...
                //Yearly
                var YArryItem = [];
                for (var i = 0; i < response.approvedOrderYearWise.length; i++) {
                    YArryItem.push(response.approvedOrderYearWise[i].ordersCount);
                }
                var YNArryItem = [];
                for (var i = 0; i < response.approvedOrderYearWise.length; i++) {
                    YNArryItem.push(response.approvedOrderYearWise[i].yearNo);
                }

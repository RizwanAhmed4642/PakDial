$(function () {
    ...
    $('#AdminSEOrder').DataTable({
        ...
        "order": [[0, "desc"]],
        ajax: {
            url: "/AdminSeOrders/LoadSeOrders",
            type: "POST",
            datatype: "json",
        ...
        columnDefs: [
            ...
            { "width": "5%", "targets": 8 },
            {
                ...
                render: function (data, type, full, meta) {
                    var Id = full.id;
                    var Invoice = '<button id="SeOrderGenrateInv" value=' + Id + ' type="button" class="btn mr-1 mb-1 btn-primary btn-sm">&nbsp;Invoice&nbsp;</button>'
                    var Collect = "";
                    if (full.statusName == "Pending") {
                        Collect = '<button id="SeOrderCollectInv" value=' + Id + ' type="button" class="btn mr-1 mb-1 btn-danger btn-sm">&nbsp;Collect&nbsp;</button>'
                    }
                    return '<td> ' + Invoice + Collect + ' </td>'
                }
            }
        ],
    ...
    $("#AdminSEOrder").on('click', '#SeOrderGenrateInv', function () {
        var invoiceNo = $(this).val();
        window.location.href = "/AdminSeOrders/SaleExReport?InvoiceId=" + invoiceNo
    });
    $("#AdminSEOrder").on('click', '#SeOrderCollectInv', function () {
        var invoiceNo = $(this).val();
        window.location.href = "/AdminSeOrders/GetCollectSaleExOrder?InvoiceId=" + invoiceNo
    });
